...
--
-- ATENÇÃO: as colunas de referência abaixo assumem que projetos.id e roadmaps.id
-- são do tipo "bigint" (padrão do Supabase para tabelas criadas pelo editor).
-- Se suas chaves primárias forem "uuid", troque "bigint" por "uuid" nas linhas
-- marcadas com REFERENCES antes de rodar.
...
alter table roadmaps add column if not exists projeto_id bigint references projetos(id) on delete set null;
...
alter table lancamentos add column if not exists projeto_id bigint references projetos(id) on delete set null;
